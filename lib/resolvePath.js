...
function findPathInRoots(sourcePath, {
  ...
  root.some(basedir => {
    if (isModulePath) {
      ...
      resolvedSourceFile = _path.default.resolve(basedir, `./${sourcePath}`);
    }
...
function resolvePathFromAliasConfig(sourcePath, currentFile, opts, {
  ...
  if (Array.isArray(aliasedSourceFile)) {
    return aliasedSourceFile.map(asf => {
      ...
      return !isModulePath || (0, _utils.nodeResolvePath)(src, _path.default.dirname(currentFile), opts.extensions);
    });
  }
